...
public:
    long long minimumCost(string source, string target, vector<char>& original, vector<char>& changed, vector<int>& cost) {
        ...
        int n=26;
        vector<vector<ll>> vec(26,vector<ll>(26,INT_MAX));
        long long ans=0;
        for(int i=0;i<original.size();i++)
        {
            vec[original[i]-'a'][changed[i]-'a']=min(vec[original[i]-'a'][changed[i]-'a'],(ll)cost[i]);
        }
        loop(i,0,n)
        {
            loop(j,0,n)
     ...
     if(vec[j][i]!=INT_MAX && vec[i][k]!=INT_MAX)
     {
        ...
        for(int i=0;i<source.size();i++)
        {
            ...
            {
                if(vec[source[i]-'a'][target[i]-'a']>=INT_MAX)
                {
                    return -1;
                    
                }
                ans+=vec[source[i]-'a'][target[i]-'a'];
            }
        }
        return ans;
    }
